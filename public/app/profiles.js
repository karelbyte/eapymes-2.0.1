app.controller('profiles_ctrl', ['$scope', '$http', '$resource', 'restAPI',  'userauth',function ($scope, $http, $resource, restAPI, userauth ) {
    $scope.createtpl = "/tpl/profiles/create.blade.php";
    ...
    function showhelp() {
        $http({
            ...
            if ( !response.data)   $scope.helptpl = "/tpl/profiles/help.php";
        })
    }
        ...
        switch (modalstate) {
            case 'add':
                if (id == null){
                    ...
                    restAPI.rest('/profiles').get({id: id}).$promise.then(function(response){
    ...
    $scope.save = function(modalstate, id) {
        angular.copy({}, $scope.retorno);
        if (modalstate === 'edit'){
            restAPI.rest('/profiles').update({id: id}, $scope.entity).$promise.then(function successCallback(response) {
            ...
            restAPI.rest('/profiles').save($scope.entity).$promise.then(function successCallback(response) {
                alertas("#msj-success", response, "#name");
                $scope.getresult($scope.currentpage);
            }, function errorCallback(msj) {
                $scope.retorno = msj.data;
    ...
    $scope.delete = function(){
        restAPI.rest('/profiles').delete({id:$scope.kill}).$promise.then(function(response){
